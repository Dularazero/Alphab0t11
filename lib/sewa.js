...
const expiredCheck = (alpha, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            console.log(color('Please use pm2 so that bots don't turn off automatically','yellow'))
            if (_dir[position].status === true){
                alpha.sendMessage(_dir[position].id, `The rental time in this group has expired, the bot will exit automatically.`, MessageType.text)
                .then(() => {
